...
const KANBAN_TITLES = [
  // Infra realista
  'Investigate memory leak in auth-service','Update SSL certificates for api-gateway',
  'Refactor database connection pool logic','Deploy hotfix to production us-east-1',
  'Add Redis caching layer to user sessions','Monitor Kafka lag on consumer group B',
  'Rollback failed canary in eu-west-1','Tune JVM heap size for billing-worker',
  'Set up new Grafana alerts for latency','Validate backup restoration pipeline',
  'Review infrastructure cost report Q3','Migrate legacy logs to S3 Glacier',
  'Patch CVE-2024-XXXX in nginx ingress','Optimize Elasticsearch index mapping',
  'Implement circuit breaker for payments','Shard PostgreSQL by tenant_id',
  'Canary deployment stuck at 4% traffic','Rotate Datadog API keys',
  'Evict cold objects from CDN edge nodes','Rebalance MongoDB replica set',
  'Audit IAM roles with last-used > 90 days','Fix WebSocket close-code propagation',
  'Investigate tail latency on p99.9','Enable connection pooling in sidecar',
  'Dry-run terraform plan for eu-north-1','Replay dead-letter queue #DLQ-A',
  // Global monitoring
  'Cross-reference shipping manifests with weather','Flag anomalous BGP route advertisements',
  'Log arctic ice sensor drift','Report ocean acidity spike near ap-south-1',
  'Annotate seismic activity near us-west-2','Validate satellite downlink parity',
  // Absurdo / vigilancia corporativa
  'Recalibrate breakroom coffee weight sensors','Audit ficus #12 leaf-drop rate anomaly',
  'Reduce open-office conversational decibel ceiling','Wellness check on cars in lot C since Tuesday',
  'Deploy praise algorithm to plant #7','Investigate keyboard acoustics in pod 4B',
  'Tune dopamine delivery pipeline for cat videos','Rollback citizen mood classification to v2.1',
  'Update billboard smile-detection threshold','Refactor umbrella-prediction model for District 7',
  'Patch printer grief protocol on HP-7F','Monitor vending machine tuna sandwich returns',
  'Validate restroom visit productivity metric','Implement ghost-ping auto-response handler',
  'Recalibrate eyeball dwell time on Ad-Board-7','Deploy yawn-suppression audio in elevators',
  'Recalibrate seagull proximity sensors','Audit Slack emoji sentiment baseline',
  'Reduce calendar focus-block anomaly score','Tune recycled-air happiness index',
  'Investigate parking karma imbalance','Patch predictive lunch-break model',
  'Audit thermostat democracy vote weights','Recalibrate ghost-ping tolerance window',
  'Validate taco-truck proximity correlation','Tune involuntary-smile revenue attribution',
  'Investigate 3D-printer existential dread','Rollback pineapple-pizza sentiment drift',
];
...
const ACTIVITY_ACTIONS = [
  ...
  'synchronized configuration','rolled back canary','scaled nodes horizontally',
  'invalidated cache','rotated secrets','tuned garbage collector',
  'replayed DLQ','audited IAM policy','patched kernel CVE',
  'evicted pod','rebuilt index','flushed connection pool',
  // Global monitoring
  ...
  'tracked shipping route','validated satellite parity','logged seismic tremor',
  'reported arctic melt rate','flagged ocean acidity','noted aurora interference',
  // Absurdo / vigilancia corporativa
  'calibrated coffee stockpile sensors','investigated ficus #12 morale index',
  'patched printer grief protocol','updated billboard smile count',
  'approved restroom break efficiency','recycled air intake quota met',
  'tuned predictive umbrella model','logged ghost ping duration',
  'calibrated keyboard acoustics threshold','reset dopamine delivery funnel',
  'measured yawn frequency','audited taco-truck proximity',
  'validated parking karma score','recalibrated thermostat democracy',
  'logged involuntary smile revenue','tuned seagull migration alert',
  'measured focus-block suspicion','approved pineapple-pizza delisting',
  'patched ghost-ping auto-response','recycled air happiness check passed',
];
...
const ACTIVITY_TARGETS = [
  ...
  '#CLUSTER-A','#POOL-B','#CANARY-4','#DLQ-A',
  '#IAM-ROT','#CVE-2024','#KERN-77','#INDX-B',
  '#POD-99','#GC-TUNE','#WSOCK-3','#TF-PLAN',
  // Global / data
  '#BGP-RT','#ICE-DRIFT','#OCEAN-PH','#SAT-DL',
  '#SEIS-42','#SHIP-MAN','#AUR-OSL','#ARCTIC-4',
  // Absurdo / vigilancia
  '#CAF-991','#FIC-12','#HP7F-JAM','#ADM-7',
  '#DOP-231','#UMB-D7','#GHOST-47','#ACOU-4B',
  '#REST-Q3','#AIR-18K','#BATH-08','#SMILE-3',
  '#YAWN-09','#TACO-400','#KARMA-12','#THERM-22',
  '#REV-EUR','#PINE-18','#SGULL-47','#FOCUS-4B',
];

const ALERT_TITLES = [
  // Infra realista
  'Critical latency spike on api-gateway','High memory usage on worker nodes',
  'Database connection pool exhausted','Cache miss rate above threshold',
  'Failed health checks on service mesh','Disk usage > 85% on log aggregators',
  'Kafka consumer lag increasing','SSL certificate expiring in 7 days',
  'Unusual CPU pattern detected','Replication lag on primary DB',
  'WebSocket close-code storm in us-east-1','Canary rollback timeout exceeded',
  'Dead-letter queue depth > 10k','Terraform plan diff unexpected',
  'BGP route flap detected','Pod eviction loop on node-14',
  'Garbage collection pause > 200ms','IAM role assumption anomaly',
  // Global / data
  'Solar flare Kp-index above 6','Arctic sensor drift > 2%',
  'Shipping manifest mismatch SUEZ','Seismic tremor near cable landing',
  'Ocean acidity spike SE-ASIA','Satellite downlink checksum failed',
  // Absurdo / vigilancia corporativa
  'Coffee stockpile below 18 cups equivalent','Ficus #12 leaf-drop rate up 34%',
  'Keyboard acoustics flagged in pod 4B','Yawning cascade detected in APAC',
  'Vending machine sentiment critical (Snack D-7)','Office chair occupancy variance down 12%',
  'Ghost ping unresolved for 47 minutes','Restroom visit variance exceeds Z-score -1.8',
  'Predictive umbrella model confidence below 85%','Dopamine delivery pipeline stalled',
  'Seagull migration anomaly near exhaust vents','Calendar density alert: 4 focus blocks',
  'Slack emoji velocity below 0.3 threshold','Taco-truck proximity causing productivity dip',
  'Parking lot wellness check triggered','Printer HP-7F jam count > 14 today',
  'Thermostat democracy vote tampering suspected','Involuntary smile revenue below €0.05/hr',
];
...
const ALERT_DESCS = [
  ...
  'WebSocket clients receiving 1006 close codes at 340/min.',
  'Canary stuck at 4% for 18 minutes. Automatic rollback initiated.',
  'DLQ depth reached 10,847 messages. Replay worker capacity insufficient.',
  'Terraform diff shows 7 unexpected security group changes.',
  'BGP neighbor flapped 12 times in 3 minutes. Route dampening active.',
  'Node-14 evicting pods in loop. Kernel OOM scores misconfigured.',
  'GC pause peaked at 247ms during peak traffic. Heap tuning advised.',
  'IAM role assumed from 4 distinct ASN locations in 10 minutes.',
  // Global / data
  'Geomagnetic Kp=6.2. Satellite comms bit-error rate rising.',
  'Arctic sensor drift 2.3% vs calibration baseline. Data flagged.',
  'SUEZ manifest mismatch: 14 containers unaccounted.',
  'Seismic tremor M2.1 detected 3km from ap-south-1 cable landing.',
  'Ocean pH dropped 0.08 in 6 hours near Singapore node.',
  'Satellite downlink checksum failed on 2 of 4 channels.',
  // Absurdo
  ...
  'Seagull count near DC exhaust up 47%. Noise floor elevated 2dB.',
  'User scheduled 4 focus blocks. HR algorithm flagged as suspicious.',
  'Thumbs-up ratio fell to 0.27. Morale bot preparing intervention.',
  'Taco truck GPS ping at 380m. Productivity dip predicted in 11 min.',
  'Lot C vehicle idle for 72 hours. Wellness check dispatch queued.',
  'HP-7F jammed 14 times today. Support counselor session #3 scheduled.',
  'Thermostat vote tally shows 3.5°C swing in 2 hours. Audit opened.',
  'Ad-Board-7 smile revenue €0.042/hr. Below break-even threshold.',
];

const CHAT_MESSAGES = [
  // Infra
  'Anyone seeing elevated 500s?','Auth service looks stable now',
  'Need another pair of eyes on the PR','Redis cache seems flushed unexpectedly',
  'Gateway latency is spiking in us-west-2','Deploying hotfix to prod in 5 min',
  'DB pool issue resolved after restart','Who touched the nginx config?',
  'Throughput back to normal levels','False alarm, it was a scheduled job',
  'Merging to main, all checks passed','Nodes autoscaled to 24 instances',
  'Buffer cleared, ingestion resumed','Latency normalized after rollback',
  'Canary stuck at 4%, rolling back','GC pause hit 247ms, anyone else?',
  'WebSocket storm in us-east-1','DLQ replay running, ETA 8 min',
  'Terraform diff looks scary','Pod eviction loop on node-14',
  // Global context
  'EUR/USD volatility is wild today','Anyone monitoring the energy grid spikes?',
  'Demographic data anomaly in FR dataset','Solar flare might affect our sat links',
  'Shipping delays could impact our CDN','Commodity futures shifting, keep an eye',
  'Transit overload in BR region noted','Rainfall sensors reporting deficits again',
  'Arctic ice sensor drifting 2%','Ocean pH dropped near Singapore',
  'Seismic tremor near cable landing','BGP route flap in eu-west-1',
  // Absurdo / vigilancia corporativa
  ...
  'Seagull count near DC up 47%. Noise floor elevated.',
  'Slack thumbs-up ratio at 0.27. Morale bot incoming.',
  'Parking lot wellness check triggered for lot C.',
  'Thermostat vote shows 3.5°C swing. Audit opened.',
  'Ad-Board-7 smile revenue below break-even. Sad.',
  'Recycled air intake at 18,000L. Wellness nominal.',
  'Ghost ping unresolved 47min. Social credit -0.3.',
  'Printer grief session #3 scheduled for HP-7F.',
];
...
const NOTIFICATIONS = [
  ...
  { title: 'Canary rollback', desc: 'Canary stuck at 4% for 18 min. Automatic rollback to v2.4.0 initiated.', type: 'warn' },
  { title: 'WebSocket storm', desc: 'Close-code 1006 rate at 340/min in us-east-1. Investigating.', type: 'error' },
  { title: 'DLQ depth warning', desc: 'Dead-letter queue reached 10,847 messages. Replay worker capacity insufficient.', type: 'warn' },
  { title: 'GC pause alert', desc: 'Garbage collection pause peaked at 247ms during peak traffic.', type: 'warn' },
  { title: 'Pod eviction loop', desc: 'Node-14 evicting pods in loop. Kernel OOM scores misconfigured.', type: 'error' },
  { title: 'Terraform diff', desc: 'Terraform plan shows 7 unexpected security group changes.', type: 'warn' },
  { title: 'Secret rotation', desc: 'Datadog API keys rotated. Update your local env files.', type: 'info' },
  { title: 'Index rebuild', desc: 'Elasticsearch index rebuild completed. Search latency restored.', type: 'info' },
  // Global / data
  ...
  { title: 'Ocean acidity spike', desc: 'Ocean pH dropped 0.08 in 6 hours near Singapore node.', type: 'warn' },
  { title: 'Seismic tremor', desc: 'Seismic tremor M2.1 detected 3km from ap-south-1 cable landing.', type: 'info' },
  { title: 'BGP route flap', desc: 'BGP neighbor flapped 12 times in 3 minutes in eu-west-1.', type: 'warn' },
  { title: 'Satellite checksum fail', desc: 'Downlink checksum failed on 2 of 4 channels. Retrying.', type: 'warn' },
  // Absurdo / vigilancia corporativa
  ...
  { title: 'Taco-truck proximity', desc: 'Taco truck GPS ping at 380m. Productivity dip predicted in 11 minutes.', type: 'info' },
  { title: 'Ghost ping tolerance', desc: 'Message unread for 47 min. Social credit adjusted -0.3.', type: 'warn' },
  { title: 'Thermostat democracy', desc: 'Vote tally shows 3.5°C swing in 2 hours. Audit ticket opened.', type: 'warn' },
  { title: 'Recycled air happiness', desc: 'Air intake at 18,000L/day. Wellness index nominal.', type: 'info' },
  { title: 'Parking karma imbalance', desc: 'Taylor found first-floor spot 3 days straight. Probability audit triggered.', type: 'info' },
  { title: 'Pineapple pizza delisting', desc: 'Sentiment down 18 pts. Menu committee reviewing viability.', type: 'warn' },
];
...
function showToast(alert) {
  ...
  el.innerHTML = '<div class="toast-icon">!</div><div class="toast-content"><div class="toast-title">' + alert.title + '</div>' +
    '<div class="toast-desc">' + alert.desc + '</div></div>' +
